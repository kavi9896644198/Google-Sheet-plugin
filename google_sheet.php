 ...
 * Description: A simple plugin that adds a shortcode to display a "Hello, World!" message.
 * Version: 1.0
 * Author: Webutopian
 */
...
function fngetAcuityAppointments() {
...
//add_action('init','google_sheet_add_data_function');
function google_sheet_add_data_function(){
     // echo '<pre>';print_r(fngetAcuityAppointments());die;
       // /echo plugin_dir_path(__FILE__).'credentials.json';die;
    ...
    $acuty_data = fngetAcuityAppointments();
    if(!empty($acuty_data)){
    ...
    $existingValues = $existingData->getValues();
     // echo "<pre>";print_r($existingValues[1][4]);die;
     // echo "<pre>";print_r($newData[1][4]);die;
    // Prepare data for updating and inserting
    $dataToUpdate = [];
    $dataToInsert = [];
    foreach ($newData as $newRow) {
        //echo "<pre>";print_r($newRow);die;
        $rowExists = false;
       if(!empty($existingValues)){
        foreach ($existingValues as $key => $existingRow) {
            // echo "<pre>";print_r($existingRow);
            // echo "<pre>";print_r($key);
            if ($existingRow['4'] == $newRow['4']) {
                $rowExists = true;
                break;
            }
        }}
